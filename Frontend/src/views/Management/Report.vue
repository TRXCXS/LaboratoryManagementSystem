...
<!--                <h5 style="margin-top: 6px;">举报搜索:</h5>-->
...
<!--        </div>-->
        ...
        <el-table :data="tableData" border stripe :header-cell-class-name="headerBg">
            ...
            <el-table-column prop="user_id" label="举报人ID">
            </el-table-column>
            <el-table-column prop="seat_id" label="椅子ID">
            </el-table-column>
...
<!--            </el-table-column>-->
            ...
            <el-table-column label="操作"  align="center">
                <template slot-scope="scope">
                    <el-button type="info" @click="handleAdd(scope.row.report_id)">审核 <i class="el-icon-view"></i></el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods:{
        ...
        download(photo_id){
            ...
            // var image = this.fetchBlob("http://localhost:8080/photo/"+photo_id)
            ...
            // const url = "http://localhost:8080/photo/"+photo_id;
            // const link = document.createElement('a');
            // link.href = url;
            // link.setAttribute('download', 'filename.jpg');
            // document.body.appendChild(link);
            // link.click();
            // this.request.get("/photo/"+photo_id).then(res=>{
            //     console.log(res)
                // this.$message.success("下载成功")
                //
                // const url = window.URL.createObjectURL("http://localhost:8080/photo/"+photo_id);
                // const link = document.createElement('a');
                // link.href = url;
                // link.setAttribute('download', 'filename.jpg');
                // document.body.appendChild(link);
        ...
        save(){
            ...
            this.request.post("/handling/?report_id="+this.report_id+"&confirmation="+this.form.confirmation+"&memorandum="+this.form.memorandum).then(res =>{
                if (res) {
        ...
        load(){
            this.request.get("/report/unhandled").then(res=>{
